...
Wargames.prototype.fetchIps = function () {
	...
	Object.keys(self.users).forEach(function (name) {
      ...
      if(user.host.match(/^localhost$/) || user.host.match(/^127\.0\.0\.1$/)){
        user.host = self.settings.ircNetwork;
      }
	...
	Object.keys(self.users).forEach(function (name) {
		...
		if (user.lat === undefined && user.ip) {
			(function (user) {
				...
				var lat,lng;
        var geoInfo = geoip.lookup(user.ip);
        lat = geoInfo.ll[0];
        lng = geoInfo.ll[1];
        if(lat != 0 && lng != 0){
          user.lat = lat;
...
Wargames.prototype.sendUpdate = function (from, silent) {
	...
	if (from && from.lat) {
		...
		self.server.sockets.emit('message', JSON.stringify(returnObj));
	}
	else {
...
Wargames.prototype.sendLink = function (from, to, lastDitchEffort) {
	...
	if (from && to && from.lat && to.lat) {
		self.server.sockets.emit('message', JSON.stringify({
			'action': 'newLink',
			'from': self.getPublicUserInfo(from),
